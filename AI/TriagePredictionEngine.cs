...
public class TriagePredictionEngine
{
        ...
        Action<string>? progressCallback = null)
    {
        return await Task.Run<(bool, string, double?)>(() =>
        {
            try
            {
                ...
                // Pipeline: Convert Label to Key -> Concatenate Features -> Normalize -> Train -> Convert back
    ...
    /// <summary>
    /// Detects whether the file uses tabs or commas as separators
    /// </summary>
    private char DetectSeparator(string filePath)
    {
        try
        {
            var firstLine = File.ReadLines(filePath).FirstOrDefault() ?? "";
            
            // Count occurrences of potential separators in header
